...
def _load_wechat_client():
    ...
    try:
        ...
        candidate_roots = [
            project_root / "wx4py",
            project_root.parent / "wx4py",
        ]
        repo_root = next((path for path in candidate_roots if (path / "src").exists()), None)
...
def _build_parser() -> argparse.ArgumentParser:
    """Build CLI parser."""
    parser = argparse.ArgumentParser(prog="wechat-uia", description="WeChat chat history export CLI")
